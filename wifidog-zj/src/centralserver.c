...
/** Initiates a transaction with the sub server, either to authenticate or to
 * update the traffic counters at the server
@param request_type Use the REQUEST_TYPE_* defines in centralserver.h
@param ip IP adress of the client this request is related to
@param mac MAC adress of the client this request is related to
...
*/
void
sub_server_request( const char *request_type, const char *ip, const char *mac, const char *token, unsigned long long int incoming, unsigned long long int outgoing)
{
	int sockfd;
	ssize_t	numbytes;
	size_t totalbytes;
	...
	snprintf(buf, (sizeof(buf) - 1),
		...
		sub_server->serv_auth_script_path_fragment,
		request_type,
		ip,
		mac,
		safe_token,
	...
	do {
		...
		if (nfds > 0) {
			...
			numbytes = read(sockfd, buf + totalbytes, MAX_BUF - (totalbytes + 1));
			if (numbytes < 0) {
				debug(LOG_ERR, "An error occurred while reading from sub server: %s", strerror(errno));
				/* FIXME */
				close(sockfd);
				return ;
			}
			...
			debug(LOG_ERR, "Timed out reading data via select() from sub server");
			/* FIXME */
			close(sockfd);
			return ;
		}
		else if (nfds < 0) {
			debug(LOG_ERR, "Error reading data via select() from sub server: %s", strerror(errno));
			/* FIXME */
			close(sockfd);
			return ;
		}
...
int _connect_server(int level,int type) {
	 ...
	 */
	if(type==1){
		_server= config->auth_servers;
	}
	else{
		_server= config->sub_servers;
	}

	for (; _server; _server = _server->next) {
		num_servers++;
	}
	...
	hostname = _server->serv_hostname;
	debug(LOG_DEBUG, "Level %d: Resolving  server [%s]", level, hostname);
	...
	if (!h_addr) {
		...
		if (h_addr) {
			 ...
			 */
			debug(LOG_DEBUG, "Level %d: Marking  server [%s] as bad and trying next if possible", level, hostname);
			if (_server->last_ip) {
				free(_server->last_ip);
				_server->last_ip = NULL;
			}
			if(type==1){
				mark_auth_server_bad(_server);
			}
			return _connect_server(level,type);
		}
		else {
			/*
			 * No
		...
		if (!_server->last_ip || strcmp(_server->last_ip, ip) != 0) {
			/*
			...
			if (_server->last_ip) free(_server->last_ip);
			_server->last_ip = ip;
		 ...
		 * Connect to it
		 */
		debug(LOG_DEBUG, "Level %d: Connecting to server %s:%d", level, hostname, _server->serv_http_port);
		their_addr.sin_family = AF_INET;
		their_addr.sin_port = htons(_server->serv_http_port);
		...
		if (connect(sockfd, (struct sockaddr *)&their_addr, sizeof(struct sockaddr)) == -1) {
			 ...
			 */
			debug(LOG_DEBUG, "Level %d: Failed to connect to  server %s:%d (%s). Marking it as bad and trying next if possible", level, hostname, _server->serv_http_port, strerror(errno));
			close(sockfd);
			if(type==1){
				mark_auth_server_bad(_server);
			}
			return _connect_server(level,type); /* Yay recursion! */
		}
		else {
			/*
			 * We have successfully connected
			 */
			debug(LOG_DEBUG, "Level %d: Successfully connected to  server %s:%d", level, hostname, _server->serv_http_port);
			return sockfd;
		}
	}
}

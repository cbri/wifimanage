...
iptables_fw_set_platservers(void)
{
        ...
        void
iptables_fw_clear_subservers(void)
{
        iptables_do_command("-t filter -F " TABLE_WIFIDOG_PLATSERVERS);
        iptables_do_command("-t nat -F " TABLE_WIFIDOG_PLATSERVERS);
}
        ...
        void
iptables_fw_set_subservers(void)
{
        const s_config *config;
        t_serv *sub_server;
        ...
        for (sub_server = config->sub_servers; sub_server != NULL; sub_server = sub_server->next) {
                if (sub_server->last_ip && strcmp(sub_server->last_ip, "0.0.0.0") != 0) {
                        iptables_do_command("-t filter -A " TABLE_WIFIDOG_SUBSERVERS " -d %s -j ACCEPT", sub_server->last_ip);
                        iptables_do_command("-t nat -A " TABLE_WIFIDOG_SUBSERVERS " -d %s -j ACCEPT", sub_server->last_ip);
                }
        }
...
iptables_fw_init(void)
{
	...
	for (p = config->trustedmaclist; p != NULL; p = p->next)
		iptables_do_command("-t mangle -A " TABLE_WIFIDOG_TRUSTED " -m mac --mac-source %s -j MARK --set-mark %d/0xfffffffc", p->mac, FW_MARK_KNOWN);
	...
	iptables_do_command("-t nat -N " TABLE_WIFIDOG_SUBSERVERS);
	...
	if((proxy_port=config_get_config()->proxy_port) != 0){
		...
		iptables_do_command("-t nat -A " TABLE_WIFIDOG_WIFI_TO_INTERNET " -p tcp --dport 80 -m mark --mark 0x%u/0xfffffffc -j REDIRECT --to-port %u", FW_MARK_KNOWN, proxy_port);
		iptables_do_command("-t nat -A " TABLE_WIFIDOG_WIFI_TO_INTERNET " -p tcp --dport 80 -m mark --mark 0x%u/0xfffffffc -j REDIRECT --to-port %u", FW_MARK_PROBATION, proxy_port);
	}

	iptables_do_command("-t nat -A " TABLE_WIFIDOG_WIFI_TO_INTERNET " -m mark --mark 0x%u/0xfffffffc -j ACCEPT", FW_MARK_KNOWN);
	iptables_do_command("-t nat -A " TABLE_WIFIDOG_WIFI_TO_INTERNET " -m mark --mark 0x%u/0xfffffffc -j ACCEPT", FW_MARK_PROBATION);
	...
	iptables_do_command("-t nat -A " TABLE_WIFIDOG_UNKNOWN " -j " TABLE_WIFIDOG_SUBSERVERS);
	...
	iptables_do_command("-t filter -N " TABLE_WIFIDOG_SUBSERVERS);
	...
	iptables_do_command("-t filter -A " TABLE_WIFIDOG_WIFI_TO_INTERNET " -j " TABLE_WIFIDOG_SUBSERVERS);
	iptables_fw_set_authservers();
	iptables_fw_set_portalservers();
	iptables_fw_set_platservers();
	iptables_fw_set_subservers();

	iptables_do_command("-t filter -A " TABLE_WIFIDOG_WIFI_TO_INTERNET " -m mark --mark 0x%u/0xfffffffc -j " TABLE_WIFIDOG_LOCKED, FW_MARK_LOCKED);
	...
	iptables_do_command("-t filter -A " TABLE_WIFIDOG_WIFI_TO_INTERNET " -m mark --mark 0x%u/0xfffffffc -j " TABLE_WIFIDOG_VALIDATE, FW_MARK_PROBATION);
	iptables_load_ruleset("filter", "validating-users", TABLE_WIFIDOG_VALIDATE);

	iptables_do_command("-t filter -A " TABLE_WIFIDOG_WIFI_TO_INTERNET " -m mark --mark 0x%u/0xfffffffc -j " TABLE_WIFIDOG_KNOWN, FW_MARK_KNOWN);
...
iptables_fw_destroy(void)
{
	...
	iptables_do_command("-t nat -F " TABLE_WIFIDOG_PORTALSERVERS);
	iptables_do_command("-t nat -F " TABLE_WIFIDOG_PLATSERVERS);
	iptables_do_command("-t nat -F " TABLE_WIFIDOG_SUBSERVERS);
	...
	iptables_do_command("-t nat -X " TABLE_WIFIDOG_SUBSERVERS);
	...
	iptables_do_command("-t filter -F " TABLE_WIFIDOG_PORTALSERVERS);
	iptables_do_command("-t filter -F " TABLE_WIFIDOG_PLATSERVERS);
	iptables_do_command("-t filter -F " TABLE_WIFIDOG_SUBSERVERS);
	...
	iptables_do_command("-t filter -X " TABLE_WIFIDOG_SUBSERVERS);
...
iptables_fw_access(fw_access_t type, const char *ip, const char *mac, int tag)
{
	...
	switch(type) {
		case FW_ACCESS_ALLOW:
			iptables_do_command("-t mangle -A " TABLE_WIFIDOG_OUTGOING " -s %s -m mac --mac-source %s -j MARK --set-mark %d/0xfffffffc", ip, mac, tag);
			rc = iptables_do_command("-t mangle -A " TABLE_WIFIDOG_INCOMING " -d %s -j ACCEPT", ip);
			break;
		case FW_ACCESS_DENY:
			iptables_do_command("-t mangle -D " TABLE_WIFIDOG_OUTGOING " -s %s -m mac --mac-source %s -j MARK --set-mark %d/0xfffffffc", ip, mac, tag);

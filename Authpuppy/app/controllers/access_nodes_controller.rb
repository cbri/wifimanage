...
class AccessNodesController < ApplicationController
  ...
  def AP_status
      if params[:gw_id].nil?
         back_json(4002003,"Paramters is error")
         return
      end 
      mac=params[:gw_id].gsub(/[:-]/, "").upcase
      node = AccessNode.find(:first, :conditions => ["mac = ?", mac])
      if node
         if node.running?
            back_json(200,"online")
         else
            back_json(200,"offline")   
         end
      else
         back_json(400201,"AP is not to exist")
      end
  end

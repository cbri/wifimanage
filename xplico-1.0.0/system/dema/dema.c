...
static int ReadConfigFile(char *path, dbconf *db_c, char *root_dir, time_t *twpcap)
{
    FILE *fp;
    int res, nl;
...
int main(int argc, char *argv[])
{
    ...
    if (config_file[0] != '\0') {
        /* read config file */
        if (ReadConfigFile(config_file, &db_c, root_dir, &twpcap) != 0){
            Usage(argv[0]);

...
#define  FF_BUFFER_DIM   (1024*1024)
...
multipart_f *FFormatMultipart(const char *file_name, const char *boundary)
{
    ...
    int len, rd, bdr_len, hlen, part, wlen;
    bool header;
        ...
        if (header) {
            ...
            if (tmp != NULL) {
                ...
                if (tmp != NULL) {
                    ...
                    xfree(mp->name);
                    mp->name[0] = strdup("xplico-file_without_name");
                }
            ...
            if (tmp != NULL) {
                if (mp != NULL) {
                    ...
                    if (mp->file_path != NULL) {
                        if (fpp != NULL) {
                            fwrite(buff, 1, tmp - buff, fpp);
                            fclose(fpp);
                            fpp = NULL;
                        }
                    }
                    else {
                        mp->value = xrealloc(mp->value, mp->vlen + (tmp - buff) + 1);
                        memcpy(mp->value+mp->vlen, buff, (tmp - buff));
                        mp->vlen += (tmp - buff);
                        mp->value[mp->vlen] = '\0';
                    }
                    header = TRUE;

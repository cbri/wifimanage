...
static int ProtFlowTimeout(int id)
{
    ...
    pid = FthreadFlowId(pthread_self());
...
#ifdef XPL_CHECK_CODE
    /* stack base address */
    FthreadStackBase(FthreadId(pthread_self()), &pid);
#endif
...
static void* ProtThreadFlow(void *arg)
{
    ...
    if (fid != -1) {
        ...
        if (FlowGrpIsEmpty(fid) == FALSE) {
            LogPrintf(LV_WARNING, "Dissector teminated without waiting for closure and comsume all packet in its flow (%i)", fid);
...
int ProtFlushFlow(int prot_id, int flow_id)
{
    ...
    if (j != prot_tbl[prot_id].flow_num) {
        ...
        /* flush is execute at close of flow the remove flow descritpor */
#ifdef FTBL_SORT
        prot_tbl[prot_id].flow_num--;

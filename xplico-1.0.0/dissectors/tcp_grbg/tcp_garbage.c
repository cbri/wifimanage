...
static void LoadL7Pattern(char *base, char *filename)
{
    FILE *fp;
    l7prot *proto;
    char path[TCP_GRB_FILENAME_PATH_SIZE];
...
static const char *L7Match(unsigned char *dat, unsigned int size)
{
    ...
    if (size > TCP_GRB_L7_SIZE_LIM)
        size = TCP_GRB_L7_SIZE_LIM;
...
static bool TcpGrbCheck(int flow_id)
{
    ...
    if (pkt_num > TCP_GRB_PKT_LIMIT || (pkt_num > 0 && FlowIsClose(flow_id) == TRUE)) {
        return TRUE;
    }
...
packet *TcpGrbDissector(int flow_id)
{
    ...
    ftval port_p, ip_p, port_src, port_dst, ip_src, ip_dst, lost;
    ...
    if (ipv4) {
        ...
        LogPrintf(LV_DEBUG, "TCP garbage IPv6");
    ...
    while (pkt != NULL) {
        ...
        ProtGetAttr(pkt->stk, port_src_id, &port_p);
        ProtGetAttr(ProtGetNxtFrame(pkt->stk), ip_src_id, &ip_p);
        ProtGetAttr(pkt->stk, lost_id, &lost);
        if (lost.uint8 == FALSE) {
            count++;
            end_cap = pkt->cap_sec;
#ifdef XPL_CHECK_CODE
            if (((pkt->raw + pkt->raw_len) < pkt->data)) {
                LogPrintf(LV_OOPS, "TCP data location error %p %p %lu %lu", pkt->raw, pkt->data, pkt->raw_len, pkt->len);
                ProtStackFrmDisp(pkt->stk, TRUE);
                exit(-1);
            }
            if ((pkt->data + pkt->len) > (pkt->raw + pkt->raw_len)) {
            ...
            if (pkt->len != 0) {
                if (ipv4) {
                    struct tcphdr *tcp;
                    tcp = (struct tcphdr *)(pkt->data-20);
                    seq = ntohl(tcp->seq);
                    if (FTCmp(&ip_src, &ip_p, FT_UINT32) == 0 && FTCmp(&port_p, &port_src, FT_UINT16) == 0) {
                        if (seq_s == 0)
                            seq_s = seq;
                        if (seq != seq_s) {
                            LogPrintf(LV_OOPS, "TCP src sequence error %lu/%lu 0x%x", seq, seq_s, *tcp);
                            ProtStackFrmDisp(pkt->stk, TRUE);
                            exit(-1);
                        }
                        seq_s = seq + pkt->len;
                        ...
                        if (seq_d == 0)
                            seq_d = seq;
                        if (seq != seq_d) {
                            LogPrintf(LV_OOPS, "TCP dst sequence error %lu/%lu", seq, seq_d);
                            ProtStackFrmDisp(pkt->stk, TRUE);
                            exit(-1);
                        }
                        seq_d = seq + pkt->len;
                    ...
                    LogPrintf(LV_FATAL, "Check for IPv6 to be implemented (%d)", __FUNCTION__);
                    exit(-1);
                }
            }
#endif
...
#if GRB_FILE
            LogPrintf(LV_DEBUG, "Packet Lost");
            ProtStackFrmDisp(pkt->stk, TRUE);
#endif
        }
        if (thrs != NULL) {
...
#if 0
                    if (threshold < TCP_GRB_L7_SIZE_LIM) {
                        /* protocol type */
                        if (l7prot_type == NULL)
                            l7prot_type = L7Match((unsigned char *)pkt->data, pkt->len);
                    }
#endif
                    memcpy(thrs+threshold, pkt->data,  pkt->len);
                    threshold += pkt->len;
...
int DissectInit(void)
{
    ...
    prot_list = NULL;
    dir_pat = TCP_GRB_L7_PROT_DEFAULT;
    dirp = opendir(dir_pat);
    if (dirp == NULL) {
        dir_pat = TCP_GRB_L7_PROT_INSTALL;
        dirp = opendir(dir_pat);
    }
    if (dirp == NULL) {
    ...
    while ((dp = readdir(dirp)) != NULL) {
        if (dp->d_name[0] == '.')
            continue;
        else if (strstr(dp->d_name, TCP_GRB_L7_EXTENSION) == NULL)
            continue;

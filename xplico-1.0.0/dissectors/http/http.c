    ...
    do {
        ...
        if (end != NULL) {
            ...
            if (req == TRUE) {
                ...
                HttpMsgInit(rex->msg);
                msg = rex->msg;
                msg->mtd = HttpReqMethod(rex->hdr_buf, rex->hdr_sz);
...
static bool HttpVerifyCheck(int flow_id, bool check)
{
    ...
    if (pkt != NULL) {
        ...
        ProtGetAttr(pkt->stk, lost_id, &lost);
        if (lost.uint8 == TRUE && check == FALSE) {
            /* only if there is a packet lost in head ho stream and port
               of server is 80, then we try to identify http */
            PktFree(pkt);
            pkt = FlowGetPktCp(flow_id);
            if (pkt != NULL) {
                ProtGetAttr(pkt->stk, lost_id, &lost);
            ...
            do {
                ...
                if (lineend != data+len && (*eol == '\r' || *eol == '\n')) {
                    ...
                    if (ver != HTTP_VER_NONE) {
                        if (HttpReqMethod(data, lineend-data) != HTTP_MT_NONE) {

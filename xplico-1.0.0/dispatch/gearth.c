...
static inline int GearthCoord(const pstack_f *stk, gea_coord *a, gea_coord *b, int *ca, int *cb)
{
    ...
    if (ip != NULL) {
        ...
        *ca = GeoIPLocIP(&val, FT_IPv4, &a->latitude, &a->longitude);
        FTFree(&val, FT_IPv4);
#if GEA_PRIVATE_NET
        if (*ca == -1) {
            a->latitude = lat;
        ...
        *cb = GeoIPLocIP(&val, FT_IPv4, &b->latitude, &b->longitude);
        FTFree(&val, FT_IPv4);
    }
    else {
        ...
        if (ip != NULL) {
            ...
            *ca = GeoIPLocIP(&val, FT_IPv6, &a->latitude, &a->longitude);
            FTFree(&val, FT_IPv6);
            ProtGetAttr(ip, ipv6_dst_id, &val);
            *cb = GeoIPLocIP(&val, FT_IPv6, &b->latitude, &b->longitude);
            FTFree(&val, FT_IPv6);
        }
        else {
            return -1;

...
static MYSQL *mysql_db;                   /* sqlite DB */
...
static int DispHttp(pei *ppei)
{
    ...
    unsigned long id;
    char *host, *host_t, *urlp, *path,*eth_src;
    char *file, *filename;
    ...
    /* search eth */
    frame = ProtStackSearchProt(ppei->stack, eth_id);
    mac_str[0]="\0";
    if (frame) {
        ProtGetAttr(frame, eth_src_id, &val);
    ...
    /* compose http query and insert record */
        ...
        DispFlowInfo(flow_info, ppei->stack);
        char urlmac[2*XS_QUERY_DIM];
        sprintf(urlmac, "%s&%s",url, mac_str);
        /* query */
        sprintf(query, XS_QUERY_WEB_TEMPLATE, sess, pol, src_id, PEI_TIME(ppei->time_cap), flow_info,
                urlmac, XS_URL_REL_6, method, response, agent, host, cont_type, rq_header, rq_body, rq_bd_size, rs_header, rs_body, rs_bd_size);
        if (DispQuery(query, &id) != 0) {
            printf("query: %s\n", query);
        }
        else {
...
static int DBInit(void)
{
    ...
    if (mysql_db == NULL) { // 如果返回NULl说明初始化失败
    ...
    mysql_db = mysql_real_connect(mysql_db, "42.123.76.20", "root", "ctbri@2014", "xplico", 0, NULL, 0);
...
int DispInit(const char *cfg_file)
{
    ...
    eth_id = ProtId("eth");
    if (eth_id != -1) {
        eth_src_id = ProtAttrId(eth_id, "eth.src");
    }
